...
\[
\sum_{i=1}^n m_i \Delta x_i \leq \sum_{i=1}^n f(\xi+i) \Delta x_i \leq \sum_{i=1}^n M_i \Delta x_i
\]
...
\[
\underline{S_{\tau_1}} \leq \underline{S_{\tau_2}} \leq \overline{S_{\tau_1}} \leq \overline{S_{\tau_1}}
\]
...
Пусть $m_i' = \inf_{x \in [x_{i-1}, x']} f(x), m_i'' = \inf_{x \in [x', x_{i}]}$. Тогда очевидно $m_i' \geq m_i, m_i'' \leq m_i$.

В суммах $\underline{S_{\tau_2}}$ и $\underline{S_{\tau_1}}$ равны все соответствующие слагаемы, кроме тех, которые связаны с отрезком $[x_{i-1}, x_i]$.
...
\textbf{Доказательство.} Пусть $\tau$ - измельчённое разбиение и $\tau'$, и $\tau''$. Тогда $\underline{S_{\tau'}} \leq S_\tau \leq \overline{S_\tau}.$
...
\[
\overline{S_\tau} - (\overline{S_{\tau'}} + \overline{S_{\tau''}}) = M_j \Delta x_j - \left( \sup_{[x_{j-1}, c]} f(x) \right) (x-x_{j-1}) + \left( \sup_{[c, x_j]} f(x) \right)(x_j-c) \leq
\]
