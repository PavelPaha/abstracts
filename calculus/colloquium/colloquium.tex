...
\[
|\Phi(x') - \Phi(x'')| = \left| \int_0^{x'} f(t)dt - \int_0^{x''} f(t)dt \right| = \left| \int_{x''}^{x'} f(t)dt \right| \leq M(x'-x'') 
\]

Сейчас мы хотим сделать $M(x'-x'') < \varepsilon$. Поэтому сделаем $\delta < \frac{\varepsilon}{M}$
